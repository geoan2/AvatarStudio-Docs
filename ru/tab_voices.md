...
Voice Assets поддерживают **автоматическую генерацию лип-синка** через внешний инструмент на базе **MFA**.

> [!IMPORTANT]
> MFA обработка происходит через **отдельную Python программу** (распространяется отдельно от плагина). Плагин экспортирует данные, внешний инструмент обрабатывает их через MFA, и плагин импортирует готовые результаты.

### Workflow автогенерации

```mermaid
graph LR
    A[Voice Asset] --> B[Export for MFA]
    B --> C[Audio + Text Files]
    
    C --> D[External Python Tool]
    D --> E[MFA Analysis]
    E --> F[JSON Results]
    
    F --> G[Import from MFA]
    G --> H[Lipsync Data Asset]
    
    H --> I[Morph Target Animation]
...
### Шаг 1: Export for MFA

1. Откройте Voice Asset во вкладке **Voices**
2. Нажмите кнопку **"Export for MFA"**
3. Выберите папку для экспорта
4. Система создаст файлы для каждой реплики:
   - `{LineID}.wav` — аудио файл
   - `{LineID}.txt` — текст субтитров

### Шаг 2: Обработка во внешнем инструменте

1. Запустите **Python программу** для MFA обработки
2. Укажите папку с экспортированными файлами
3. Программа обработает аудио через MFA
4. Получите JSON файлы с результатами:
   - `{LineID}.json` — данные фонем и слов
...
- `words[]` — временные метки слов
  - `start` / `end` — время в секундах
  - `text` — текст слова
- `phonemes[]` — временные метки фонем (IPA символы)
  - `start` / `end` — время в секундах
  - `text` — IPA символ фонемы

### Шаг 3: Import from MFA

1. Вернитесь в плагин
2. Нажмите кнопку **"Import from MFA"**
3. Выберите папку с JSON файлами
4. Система автоматически:
   - Найдет `LipsyncDataAsset` для каждой реплики (по `LineID`)
   - Импортирует данные слов и фонем
   - Определит язык автоматически (русский/английский)
   - Конвертирует IPA фонемы в морф-таргеты
   - Создаст анимационные кривые

### Автоматическое определение языка

Система анализирует фонемы и определяет язык:

| Язык | Маркерные фонемы | Пример |
|------|------------------|--------|
| **Русский** | `ɨ`, `ʂ`, `ʐ`, `ʲ` | "мне" → `ɲ`, `i` |
| **Английский** | Все остальные | "hello" → `h`, `ɛ`, `l`, `oʊ` |

### Конвертация фонем в морф-таргеты

Система использует **Language Profiles** для маппинга IPA → Visemes:

```cpp
// Пример маппинга для русского языка
ɛ → v_E
a → v_Ah
o → v_O
i → v_I
...
После импорта каждая реплика получает:
- ✅ `LipsyncDataAsset` с данными фонем
- ✅ Готовые анимационные кривые для морф-таргетов
- ✅ Временные метки слов для субтитров
- ✅ **Персонаж готов к озвученным диалогам!**
